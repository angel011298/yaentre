...
pnpm admin:notes-rls          # admin_notes: STUDENT/anónimo no leen, listan ni escriben, por efecto
pnpm admin:notes-cross-device # EN PRODUCCIÓN: una nota creada en un contexto aparece/se borra en otro
...
- ❌ **Un carácter de control invisible dentro de una regex la deja muerta sin que nada lo delate.** `C-sin-id-en-el-borde` —la sonda que verifica el guardrail más importante de toda la capa de autorización: «ningún esquema del borde acepta un id de usuario»— llevaba **inerte desde G65 (`069026a`)**. Su patrón contenía un **BACKSPACE REAL (U+0008)** donde se quiso escribir `\b`: alguien tecleó la secuencia de escape y lo que se guardó fue el carácter de control, invisible en el editor, en `git diff` y en `grep`. El patrón exigía un backspace literal delante de `userProfileId`, cosa que no existe en ningún código fuente, así que `sospechoso.test('userProfileId: x')` devolvía **false** y la sonda informaba «0 aceptan un id de usuario» SIEMPRE — revisara 22 archivos o 141, hubiera ofensores o no. Dos reglas: **(a)** cuando una comprobación nunca ha estado en rojo, sospechar de ella y forzarle un rojo (aquí: añadir `userProfileId` a un esquema cualquiera debe nombrarlo); **(b)** su barrido incluye ahora `src/lib/**` además de `app/actions` y `app/api`, porque los esquemas del borde no siempre viven junto a su acción y mover un archivo de carpeta bastaba para salirse del control. El `.slice(1)` sobre `split('z.object(')` tampoco es cosmético: sin él, un archivo SIN esquemas se analiza entero y cualquier `where: { userProfileId }` de Prisma se cuenta como ofensor (24 falsos positivos al arreglar la regex).

- ❌ **Un `.html` en la bóveda nunca se sirve como `text/html` EN LÍNEA — eso sería self-XSS en la sesión más privilegiada del producto.** `.html` SÍ está en la lista blanca (`VAULT_TYPES`) porque se pidió explícitamente poder subirlo, pero `vaultResponseHeaders` fuerza `Content-Type: text/plain; charset=utf-8` cuando se sirve `inline` (Ver), aunque el tipo real guardado sea `text/html`. Si se sirviera con su tipo real e `inline`, navegar DIRECTO a `/api/admin/vault/<id>` —nada lo impide, es la misma URL que usa el visor— haría que el navegador ejecutara cualquier `<script>` del archivo con el origen `https://yaentre.com` y la sesión del admin que lo abre: la cookie es httpOnly así que un script no puede leerla, pero SÍ puede usarla — un `fetch()` del propio script hacia cualquier Server Action del panel viaja con esa sesión adjunta. Un ADMIN no maestro podría subir un `.html` que invoque `changeRoleAction` en cuanto lo abriera el admin MAESTRO: la escalada de privilegios exacta que la compuerta de maestro existe para impedir. Al DESCARGAR (`?download=1`) sí se sirve el tipo real, porque `Content-Disposition: attachment` obliga a GUARDAR, no a interpretar — abrirlo después ocurre fuera del origen y la sesión de la app. `previewKindFor('text/html')` devuelve `'text'`: se ve como código fuente escapado, igual que `.txt`/`.md`, nunca como página. Probado por mutación en `tests/admin/vault.test.ts`.

- ❌ **Un apartado nuevo del panel que tenga que sobrevivir «a un dispositivo nuevo, en el momento que sea» no puede tocar `localStorage`.** Las Notas de `/admin/boveda` (tabla `admin_notes`, migración 0018) se leen y escriben ÚNICAMENTE por Prisma, detrás de `requireRole('ADMIN')` — ni una línea de `localStorage`/`sessionStorage`. Se verificó de la forma más literal posible: `scripts/g99/notes-cross-device-probe.ts` abre TRES contextos de Playwright completamente independientes (sin cookies ni storage compartido entre sí — el equivalente real a "otro dispositivo"), cada uno con su propio login, y confirma que una nota creada en el primero es visible en el segundo, se borra desde el segundo, y un tercero confirma que ya no está. Cualquier ADMIN (no hace falta maestro) puede crear, ver y borrar — son apuntes internos de bajo riesgo, a diferencia de una cortesía o un cambio de rol.

- ❌ **Dos limitadores de login distintos, y solo UNO se puede limpiar por SQL.** Al verificar `notes-cross-device-probe.ts` con varias corridas seguidas, el `SIGN_IN` de Postgres (`app_security.rate_limit_hits`, 8/10 min) se limpió a mano y la sonda SIGUIÓ bloqueada, con el mensaje en pantalla «Demasiados intentos de inicio de sesión. Espera 1 minuto» — es el limitador EN MEMORIA de `proxy.ts` (primera línea, barata, pero que **no cuenta entre instancias de Vercel**, ver el guardrail de G65 §5 sobre `src/lib/rate-limit/limiter.ts`), y ese no tiene fila que borrar: solo se libera esperando su propia ventana. Una sonda que hace logins repetidos en ráfaga para depurarse a sí misma puede agotar ese limitador sin haber tocado nada del `SIGN_IN` compartido — el síntoma en pantalla (el TEXTO del error, no un timeout ciego) es lo que distingue una cosa de la otra.

- ❌ **Un `fill()` de Playwright puede aterrizar en el DOM sin que React se entere.** `textarea.fill(texto)` deja el VALOR visible de inmediato, pero si cae antes de que React adjunte su `onChange` (mismo hidratación-race que el botón `type="submit"` de G99), el estado controlado (`content`) se queda vacío y un botón con `disabled={... || content.trim().length === 0}` nunca se habilita — sin lanzar ningún error, así que parece que el `fill()` "no sirvió". Un `textarea.waitFor({ state: 'visible' })` + un margen fijo ANTES del primer `fill()` (no reintentar el `fill()` a ciegas: eso no cambia nada si el verdadero problema es el momento del primer intento) resuelve la misma familia de carrera que ya obligó a reescribir el formulario de subida de la bóveda como botón `type="button"`.
