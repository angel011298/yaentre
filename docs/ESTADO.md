...
contenido; el cierre de G54 anunciaba «Sonnet 4.6» — **décimo ciclo** plan ≠ real). **COMPLETADA.
35 reactivos insertados con `isVerified=false`** en la cola de verificación ciega. Registro
permanente: `docs/content-batches/g55-ipn-fismat-matematicas.json`.

### 1) El encargo y la decisión de materia (con números reales)

Igual que G53: 35 reactivos para **la materia de mayor brecha efectiva pendiente entre las
instituciones activas del launch** (UNAM Superior + IPN Superior), con la metodología de G26:

- densidad = `GOAL_VERIFIED / Σ questionWeight` = `1 500 / 270` = **5.5556 reactivos por punto
  de peso**;
- meta de un pool compartido = meta de su **celda de mayor peso** (`computeSharedGoal`).

Brecha (`meta − verificados`) por pool, solo UNAM + IPN Superior, consulta en vivo a Supabase
(2026-08-31, tras la verificación ciega de G54):

| Pool | Peso | Meta | Verificados | **Brecha** |
|---|---|---:|---:|---:|
| **IPN:MATEMATICAS** (FISMAT w24 + MEDBIO w8) | — | **133** | 70 | **63** |
...
**Elegida: IPN:MATEMATICAS** (`subjectId cmrr1kvuv009dhi3nrfzki2c1` de la celda FISMAT,
`questionWeight` **24**, `sharedContentKey` **IPN:MATEMATICAS**). Es la mayor brecha (63). **Pool
COMPARTIDO** → la regla (CLAUDE.md / G26 §2) es insertar contra el `topicId` de la materia con
más contenido del grupo: FISMAT **70 verificados** vs MEDBIO **0**, así que el lote va a los 12
temas de FISMAT y `loadEquivalentSubjectIds` lo sirve también a MEDBIO. El pool combinado (105)
sigue por debajo de la meta 133, así que el lote **descuenta brecha efectiva ~1:1** cuando G56 lo
verifique (caso favorable, patrón G45/G53, opuesto a G51/G52 donde el pool ya estaba rebasado).
Lotes previos de la materia: **G3a** y **G13** (→ G3b, G14). Follow-up G26 §8.3 sin cerrar:
ampliar el temario del seed de MEDBIO Matemáticas (hoy 6 temas).

### 2) Reparto — priorizar los temas de menor cobertura

Consulta por tema antes del lote: los 12 temas de FISMAT Matemáticas en
**7/6/7/6/5/6/6/5/5/5/6/6** verificados (total 70). Reparto del lote (mínimo 2 por tema; +4 a los
cuatro temas en 5✓; +3 a tres de los seis temas en 6✓; +2 a los dos temas en 7✓ y a los demás):

| Tema | `topicId` | Antes → después | Nuevos | Anclaje |
|---|---|---:|---:|---|
| Números y operaciones | `cmrr1kw1c009fhi3nuw7hczl9` | 7 → 9 | 2 | SOURCED |
| Álgebra elemental | `cmrr1kwvw009hhi3nr37qipkh` | 6 → 8 | 2 | TEMARIO |
| Ecuaciones lineales y cuadráticas | `cmrr1kxll009jhi3n8g2zoku1` | 7 → 9 | 2 | SOURCED |
| Funciones y gráficas | `cmrr1ky75009lhi3nez0n14di` | 6 → 9 | 3 | TEMARIO |
| Trigonometría | `cmrr1kyv6009nhi3nt4xnrerr` | 5 → 9 | 4 | TEMARIO |
| Geometría analítica | `cmrr1kzda009phi3nj9dflifk` | 6 → 9 | 3 | TEMARIO |
| Cálculo diferencial | `cmrr1kzty009rhi3n401xbyy6` | 6 → 9 | 3 | TEMARIO |
| Cálculo integral | `cmrr1l0g1009thi3nvhn52qr5` | 5 → 9 | 4 | TEMARIO |
| Matrices y sistemas | `cmrr1l0vj009vhi3ntaaehqnc` | 5 → 9 | 4 | TEMARIO |
| Combinatoria y probabilidad | `cmrr1l1ej009xhi3nm9rpwlhs` | 5 → 9 | 4 | TEMARIO |
| Sucesiones y series | `cmrr1l1vr009zhi3ns2t44soo` | 6 → 8 | 2 | SOURCED |
| Números complejos | `cmrr1l2f500a1hi3nanpwcnyb` | 6 → 8 | 2 | TEMARIO |

### 3) Anclaje — 6 SOURCED / 29 TEMARIO_ONLY

`loadTopicChunks` devolvió `SourceChunk` en **3 de los 12 temas**, y `grounding.ts` hace
obligatoria la cita cuando el tema tiene fragmento (por eso los 6 reactivos de esos 3 temas son
todos SOURCED, no una elección):

- **Números y operaciones** — `uam_csh.pdf` p. 36 (porcentaje de un porcentaje, proporción,
  interés). NO1 lleva el «% de un %» a un caso nuevo; NO2 transforma la proporción **directa** de
  la guía en una de **proporción inversa** (obreros/días).
- **Ecuaciones lineales y cuadráticas** — `ceneval_exanii_i.pdf` p. 18 (ejemplo del Nuevo
  EXANI-I: intersección de la parábola `y = x² + 11x + 28` con el eje `x`, con la argumentación
  de cada distractor). EC1 usa la argumentación del distractor A de la guía («considera el 4 como
  raíz y olvida igualar a 0») como pregunta de **análisis de error**; EC2 convierte el error del
  distractor C («confunde con el eje y») en la **pregunta directa** por el corte con el eje `y`.
- **Sucesiones y series** — `uam_cad.pdf` p. 18 (completar la sucesión aritmética `32, 34, 36,
  38, 40` por paridad y diferencia común). SS1 pasa de completar a hallar el **término general**
  `aₙ = 2n + 30`; SS2 hace del rasgo definitorio (diferencia constante vs cociente constante) la
  pregunta, con «todos pares» como distractor-trampa (rasgo verdadero del ejemplo, no la
  definición).

Procedencia UAM/CENEVAL — otras instituciones que la del examen destino; se anota (patrón
G18/G33/G45/G49/G53). Los otros 9 temas no tienen fragmento → **TEMARIO_ONLY**, redactados desde
el temario oficial de FISMAT.

### 4) Verificación de cálculo (criterio del encargo)

**Matemáticas es `isCalcSubject`** (`CALC_SUBJECT_KEYS` incluye `matematica`, `algebra`,
`calculo`), así que el candado de G28 aplicará a G56. **18 de los 35 son numéricos** y se
recalcularon desde cero con **sympy** por una ruta **independiente** de la del enunciado
(`sp.integrate` para las integrales, `sp.solve` para los sistemas y raíces, `Matrix.det` para el
determinante, `factorial` para las permutaciones), exigiendo para cada uno: (a) el valor coincide
con la opción marcada correcta, (b) **coincidencia única** contra las 4 opciones (tolerancia
relativa 2 %), (c) opciones numéricas en orden ascendente estricto — **18/18 en verde**.

Ejemplos de lo verificado: `0.36 × 5/6 = 0.30`, proporción inversa `6·15/9 = 10`, `∫₁² 4x³ dx =
15`, `∫₀² (3x² − 2x) dx = 4`, `det[[3,5],[2,4]] = 2`, sistema `x+2y=9, x−y=3 → x=5`, `P(7,3) =
210`, `(1/6)² = 1/36`, `4/6 · 3/5 = 2/5`, `tan 45° = 1`, `h = 12√3·tan 30° = 12`. Cada distractor
deriva de un error de procedimiento nombrable (proporción directa donde va inversa; `(2x)²` sin
elevar el coeficiente; leer `x = 4` del factor `(x+4)`; `f(g(x))` vs `g(f(x))`; no restar `F(a)`;
suponer reemplazo; `i³ = i`), explicitado en la capa 2. Los 17 no numéricos (expresiones,
identidades, clasificación de sistemas/sucesiones, análisis de error) llevan igual una
verificación simbólica o lógica detrás.

### 5) Formato, dificultad y posición

- **Formato:** 21 `PROBLEM_SOLVING` · 11 `MULTIPLE_CHOICE` · 2 `SENTENCE_COMPLETION` · 1
  `NUMERIC_SERIES`.
- **Dificultad:** BASIC 7 · INTERMEDIATE 17 · ADVANCED 9 · EXPERT 2 (7/17/9/2, la distribución
  objetivo de `_base.md`). Los 2 `EXPERT`: la integral indefinida de `x² − 1/x²` (el doble signo
  de `∫ −x⁻² dx = +1/x`, más la trampa de leer `1/x²` como logarítmica) y la división `1/(2+i)`
  por el conjugado.
- **Clave A = 9 · B = 9 · C = 9 · D = 8** (25.7 / 25.7 / 25.7 / 22.9 %), **confirmada por query
  sobre las `options` de las 35 filas tras insertar**. Los 18 numéricos fijan la letra por el
  rango del valor correcto entre las 4 opciones ascendentes (G46 §7); los 17 restantes se
  asignaron a mano al objetivo global, con **≥ 3 letras distintas en cada tema de ≥ 3 reactivos**
  y tope 2 por letra por tema.
- **Secuencia de la clave por orden de inserción:**
  `BACBDCCDBCBDAACDCADDBCAACDBBABDBAAC` — **0 rachas cíclicas A→B→C→D de longitud ≥ 3**, racha de
  misma letra máx **2** (candado G16/G38). El orden de los reactivos dentro de cada tema se
  eligió por búsqueda aleatoria con semilla fija 55055.
- **Diagnóstico STEM (G46 §7 / G53):** rango del valor correcto entre las 4 opciones ascendentes
  = **A 5 / B 7 / C 3 / D 3** sobre los 18 numéricos (χ² ≈ 2.4, gl 3, p ≈ 0.49 — sin sesgo
  significativo, aunque `B` es la moda). Se reporta por transparencia.

### 6) Chequeos de forma (G3c / G34 §2 / G44 §6-§7 / G38 §6 / G52)

- `content:validate-batch --dir scripts/g55-lote`: **0 violaciones**
  (POSITION_SKEW/LETTER_CITATION/MALFORMED_OPTIONS/PASSAGE_LINK), antes de la DB y de nuevo como
  paso obligatorio de `content:insert --lot-dir` sobre los 12 archivos.
- **0 citas por letra** y **0 referencias posicionales** en las 105 capas (verificado por la
  regex de `lot-validation.ts` en el generador y de nuevo por query sobre las capas en la DB). Se
  reescribieron **dos fórmulas de probabilidad** (`$P(A\cap B)=P(A)\cdot P(B)$`) que la regex
  `\b[ABCD]\)` marcaba por el patrón «B)».
- **Señuelo de longitud tie-aware (G34 §2):** medido sobre los **4 ítems con opciones-oración**
  (EC1, TR4, MS4, SS2; los 31 de opción-valor/expresión/punto quedan fuera, criterio
  G45/G46/G53) → **12.5 % «más larga» / 8.3 % «más corta»**, ambos < 25 %. Se homogeneizaron a
  mano las opciones conceptuales.
- **Lenguaje absolutista (G44 §6):** 1 marcador en las 35 claves vs 1 en los 105 distractores
  («ninguna solución», «única solución» — términos matemáticos sustantivos, no retórica);
  **0/35** con la clave como única sin marcador. **Opción compuesta como única clave (G44 §7):
  0/4** conceptuales.
- **Opciones textualmente duplicadas (hueco de G52 — `lot-validation.ts` verifica
  cantidad/ids/una correcta pero NO compara los textos):** auto-chequeo en `gen.py`
  (`len(set(textos)) == 4`) → **0/35**.
- **Caracteres KaTeX:** auto-chequeo del generador → **0** ordinales `º`/`°` crudos (se usa
  `^\circ`); validación KaTeX del pipeline → **0** fragmentos `$...$` inválidos en los 35
  (matrices `\begin{pmatrix}`, sistemas `\begin{cases}`).
- **Fuga entre reactivos (G38 §6 / G42 §8):** heurístico de 4-gramas sobre `(stem + clave)` y
  sobre la diagonal `(clave ↔ distractores de todo el lote)` → **0 coincidencias no triviales**.

### 7) Inserción real — verificada en la DB

| Métrica | Antes de G55 | Después de G55 |
|---|---:|---:|
| Banco total (COUNT crudo) | 1 077 | **1 112** |
| Servibles (`isVerified=true`) | 1 073 | 1 073 |
...
| `question_source_chunks` del lote | — | **6** (2 NyO + 2 Ecuaciones + 2 Sucesiones) |
| `SOURCED` en el banco | 284 | 290 |
| IPN FISMAT Matemáticas · pool | 70✓ · 0⧗ | **70✓ / 35⧗** (⚓ del pool 20/50 → 26/79) |

Chequeos post-inserción (query directa): los 35 con exactamente 4 opciones y 1 correcta, 105
`explanation_layers`, clave A9/B9/C9/D8, dificultad 7/17/9/2, formato 21/11/2/1, reparto por tema
2/2/2/3/4/3/3/4/4/4/2/2, `groundingStatus=SOURCED` en 6 y `TEMARIO_ONLY` en 29, `verification=null`
y `isVerified=false` en los 35. Cohorte con `id` prefijo `cmth8h…`–`cmth8j…` del 2026-08-31 —
separable por `topicId` o por timestamp para G56.

`content:coverage` en vivo: **1 073 servibles · 35 pendientes · 1 retirado · 1 109 en banco**;
meta efectiva G26 (1 222) **67 %**, brecha **403 (≈ 12 lotes)** — **no se mueve con este lote**,
pero **SÍ se moverá cuando G56 verifique**: el pool IPN:MATEMATICAS (peso mayor 24, meta efectiva
133, compartido) pasa de 70 a ~105, sigue por debajo de la meta y los 35 caen enteros en la
brecha efectiva → bajará a ~368. Meta nominal de 1 500: **72 %**.

### 8) Limpieza

El lote se compuso con un generador de Python desechable en el scratchpad (`g55/gen.py`: los 35
reactivos con sus 3 capas, la verificación de los 18 cálculos con sympy por ruta independiente,
la asignación de letra, la búsqueda del orden de inserción y los auto-chequeos de distribución /
racha cíclica / señuelo de longitud tie-aware / citas por letra / absolutismo / opción compuesta
/ opciones duplicadas / caracteres KaTeX / fuga de 4-gramas). El generador y los 12 archivos del
lote **no se committean**; el registro permanente es
`docs/content-batches/g55-ipn-fismat-matematicas.json`. `pnpm typecheck` y `pnpm lint` en verde
(cero cambios de código de producción). Cero llamadas a la API de pago.

### Siguiente (G55)

1. **Verificación ciega del lote de G55** (segunda mitad del ciclo de G2), **modelo Fable 5**:
   `pnpm content:blind-batch --topic <cada uno de los 12 topicId>` (o `--all` si la cola tiene
   exactamente este lote, atajo de G54) → `content:resolve`. **LOTE DE CÁLCULO:**
   `isCalcSubject("Matemáticas")` es `true`, así que la sesión ciega recibe
   `requiresCalculation:true` y **debe EJECUTAR cada cálculo con código real** (candado de G28),
   en los **18 numéricos**. **0/35 con pasaje.** Aplicar G36 §2 / G38 §3: recalcular el señuelo
   de longitud tie-aware sobre las respuestas ciegas y compararlo con §6. **Aplicar G42 §2 / G54
   sin falta:** el `chosenOption` del archivo de respuestas se deriva de un fragmento del TEXTO
   de la opción elegida con `assert` de match único — **nunca escribir el array de letras a
   mano** (fue el fallo de G54). **Reactivos más apretados** (se señala cuáles, no cómo
   resolverlos, por G30 §1): los **2 `EXPERT`** (CI1 integral de `x² − 1/x²`; NC1 `1/(2+i)` por
   el conjugado) y **CI2** (integral definida `∫₁² 4x³ dx = 15`, donde el distractor `16` —
   «no restó `F(1)`» — queda a solo 1 unidad de la clave).
2. **Huecos que siguen abiertos** tras G55 (siguiente en brecha efectiva, tras la verificación de
   G56): **IPN:QUIMICA** (brecha 54, sirve a FISMAT y MEDBIO de un tiro); **IPN · MEDBIO ·
   Biología** (52); **IPN · FISMAT · Física** (41, era 76); **UNAM:INGLES** (33, 0 verificados,
   cubre A1/A2/A3); **IPN SOCADM** completo; **UNAM A1 Matemáticas** (114/144) y **UNAM A4
   Artes** (0, tiene `SourceChunk` en 3 temas).
3. **Auditoría 5 %: vencida, 10 ciclos.** La muestra de ids exige sesión ciega con tier **≠
   opus-5 y ≠ sonnet-5** (el banco reciente lo aprobaron esos dos).
4. **Follow-ups de código sin cerrar:** `DUPLICATE_OPTIONS` y `REASONING_LETTER_MISMATCH` y
   `NUMERIC_RANK` (G53 §5) en `scripts/lib/lot-validation.ts` / `content-resolve-verification.ts`;
   la regla de G42 §8 como código; el rebanado de `SourceChunk` por página; la rotación
   A→B→C→D de ~140 reactivos viejos; el follow-up G26 §8.3 (temario de MEDBIO Matemáticas).
...
de verificación ciega. Registro permanente: `docs/content-batches/g53-ipn-fismat-fisica.json`.

### 1) El encargo y la decisión de materia (con números reales)

El encargo pidió generar 35 reactivos para **la materia de mayor brecha pendiente entre
las instituciones activas del launch** (UNAM Superior + IPN Superior), decidiéndolo con
datos reales. Se calculó, para cada pool de materia, la brecha efectiva con la
metodología de G26:
...
- meta de un pool compartido = la meta de su **celda de mayor peso** (`computeSharedGoal`
  en `content-coverage.ts`);
- meta de una materia no compartida = `round(questionWeight × densidad)`.

Brecha (`meta − verificados`) por pool, solo UNAM + IPN Superior, consulta en vivo a
Supabase (2026-08-31):

| Pool | `questionWeight` | Meta | Verificados | **Brecha** |
|---|---|---:|---:|---:|
| **IPN · FISMAT · Física** (no compartida) | 20 | **111** | 35 | **76** |
| IPN:MATEMATICAS (FISMAT w24 + MEDBIO w8) | — | 133 | 70 | 63 |
| IPN:QUIMICA (FISMAT w10 + MEDBIO w16) | — | 89 | 35 | 54 |
| IPN · MEDBIO · Biología | 22 | 122 | 70 | 52 |
| UNAM:INGLES (A1 w6 + A2 w3 + A3 w1) | — | 33 | 0 | 33 |
...
**Elegida: Física de IPN, área FISMAT** (`subjectId cmrr1l31q00a3hi3n607qhvfu`,
`questionWeight` **20**, `sharedContentKey` **NULL**). Es la mayor brecha (76) por
cualquier lectura, y por ser materia **no compartida** el lote **descuenta brecha
efectiva 1:1** cuando G54 lo verifique (35 → 70, aún < meta 111) — el caso favorable de
G45, opuesto a G51/G52 (pool ya rebasado). Único lote previo de esta materia: **G18**
(→ G19, 35/35 auto-aprobados).

### 2) Reparto — priorizar los temas de menor cobertura

Consulta por tema antes del lote: los 14 temas de FISMAT Física en
**3/3/3/2/2/2/2/3/2/3/3/3/2/2** verificados. «Priorizar los de menor cobertura» se
resuelve nivelando: **+3 a los 7 temas en 2✓ y +2 a los 7 temas en 3✓ → los 14 quedan
en 5✓**.

| Tema | `topicId` | Antes → después | Nuevos |
|---|---|---:|---:|
| Cinemática | `cmrr1l37m00a5hi3n0nuo6d74` | 3 → 5 | 2 (SOURCED ×2) |
| Dinámica | `cmrr1l3xo00a7hi3n6w2vlzv4` | 3 → 5 | 2 |
| Trabajo y energía | `cmrr1l4k900a9hi3nvijwv2wq` | 3 → 5 | 2 |
| Momentum e impulso | `cmrr1l54400abhi3ne1tiajuj` | 2 → 5 | 3 |
| Gravitación | `cmrr1l5tg00adhi3n7c87lzal` | 2 → 5 | 3 |
| Fluidos | `cmrr1l6e100afhi3n7a0w3yvo` | 2 → 5 | 3 |
| Termodinámica | `cmrr1l6x700ahhi3nwa4pglxl` | 2 → 5 | 3 |
| Ondas y sonido | `cmrr1l7h700ajhi3n4tua0c9q` | 3 → 5 | 2 |
| Óptica | `cmrr1l7ys00alhi3nf2bgwato` | 2 → 5 | 3 |
| Electrostática | `cmrr1l8fb00anhi3nzcmxn6jy` | 3 → 5 | 2 |
| Corriente eléctrica | `cmrr1l93500aphi3nps3hcvdv` | 3 → 5 | 2 |
| Magnetismo | `cmrr1l9po00arhi3n1ay5m5kv` | 3 → 5 | 2 |
| Inducción electromagnética | `cmrr1la6500athi3nz5sx1rb1` | 2 → 5 | 3 |
| Física moderna | `cmrr1lat400avhi3nhi2mxz1n` | 2 → 5 | 3 |

### 3) Anclaje — 2 SOURCED / 33 TEMARIO_ONLY

`loadTopicChunks` devolvió `SourceChunk` solo en **Cinemática**: 1 fragmento de
`ceneval_exanii_i.pdf` p. 9 (ejemplo del Nuevo EXANI-I que distingue el concepto de
**velocidad** = razón distancia/tiempo del de **aceleración**). Procedencia CENEVAL —
otra institución que la del examen destino; se anota (patrón G18/G33/G45/G49). Los 2
reactivos de Cinemática citan `sourceChunks:[1]` (cita obligatoria vía `grounding.ts`) y
**transforman la tarea del ítem-semilla**: la guía es de identificación conceptual
(«¿qué enunciado involucra el concepto de velocidad?»); C1 la convierte en un cálculo de
rapidez media y C2 en discriminar rapidez constante (aceleración nula) de tres
escenarios con aceleración. Los otros 13 temas no tienen fragmento fuente →
**TEMARIO_ONLY**, redactados desde el temario oficial de FISMAT.

### 4) Verificación de cálculo (criterio del encargo)

**Física es `isCalcSubject`** (`CALC_SUBJECT_KEYS` incluye `fisica`,
`scripts/lib/blind-verification.ts`), así que el candado de G28 aplicará a G54. **21 de
los 35 son numéricos** y se recalcularon desde cero con Python por **dos rutas
independientes** (una sobre la lista de opciones, otra por la fórmula), exigiendo para
cada uno: (a) el valor coincide con la opción marcada correcta, (b) **coincidencia
única** contra las 4 opciones (tolerancia relativa 2 %), (c) opciones numéricas en orden
ascendente — **21/21 en verde por ambas rutas**.

Ejemplos de lo verificado: rapidez media (`d/t`), `F = ma` con `a = Δv/t` (distractor: usar
`v` en vez de `a`), `v = √(2gh)` (distractores: `2gh` y `gh` sin raíz), conservación de
momento en un retroceso, `F ∝ m/r²` con `r` al cuadrado, `ρ_cuerpo = ρ_agua · f_sumergida`,
`Q = mcΔT` con `ΔT` (no `T_inicial`), `η_Carnot = 1 − T_f/T_c`, `f = v/λ`, `f₁ = v/(2L)`
de un tubo abierto (distractor: `v/(4L)` de tubo cerrado), ecuación del espejo,
Coulomb con `r²`, `R` serie vs paralelo, `R = V²/P`, **`F = qvB·sin 0° = 0`** para una
carga paralela al campo, Faraday `ε = N·ΔΦ/Δt`, fotoeléctrico `K = hf − φ`. Cada
distractor deriva de un error de procedimiento nombrable, explicitado en la capa 2.

### 5) Formato, dificultad y posición

- **Formato:** 21 `PROBLEM_SOLVING` · 12 `MULTIPLE_CHOICE` · 2 `SENTENCE_COMPLETION`.
- **Dificultad:** BASIC 7 · INTERMEDIATE 17 · ADVANCED 9 · EXPERT 2 (7/17/9/2, la
  distribución objetivo de `_base.md`). Los 2 `EXPERT`: modo fundamental de un tubo
  sonoro abierto (ON2) y el efecto fotoeléctrico al duplicar la frecuencia (FM2 — sube
  a **más** del doble porque `hf` se duplica pero `φ` se resta una sola vez).
- **Clave A = 9 · B = 9 · C = 9 · D = 8** (25.7 / 25.7 / 25.7 / 22.9 %), **confirmada por
  `jsonb` sobre `options` de las 35 filas tras insertar**. Los 21 numéricos fijan la
  letra por el rango del valor correcto entre las 4 opciones ascendentes (G46 §7); los
  14 conceptuales se asignaron a mano al objetivo global, con **≥ 3 letras distintas en
  cada tema de ≥ 3 reactivos** y tope 2 por letra por tema.
- **Secuencia de la clave por orden de inserción:**
  `ABBABDBDCBDCBCABDACDBCACAADACDCABDC` — **0 rachas cíclicas A→B→C→D de longitud ≥ 3**,
  racha de misma letra máx **2** (candado G16/G38). El orden de los reactivos dentro de
  cada tema se eligió por búsqueda aleatoria con semilla fija 53053.

### 6) Chequeos de forma (G3c / G34 §2 / G44 §6-§7 / G38 §6 / G52)

- `content:validate-batch --dir scripts/g53-lote`: **0 violaciones**
  (POSITION_SKEW/LETTER_CITATION/MALFORMED_OPTIONS/PASSAGE_LINK), antes de la DB y de
  nuevo como paso obligatorio de `content:insert --lot-dir` sobre los 14 archivos.
- **0 citas por letra** y **0 referencias posicionales** en las 105 capas. Las 35 capa 2
  se titulan «Resolución paso a paso» / «Cómo se descarta cada opción» y describen los
  distractores por su valor o su contenido.
- **Señuelo de longitud tie-aware (G34 §2):** medido sobre los **14 ítems con
  opciones-oración** (los 21 de opción-valor quedan fuera, criterio G45/G46) →
  **21.4 % «más larga» / 21.4 % «más corta»** (3/14 y 3/14), ambos < 25 %, con varianza
  deliberada en los dos extremos.
- **Lenguaje absolutista (G44 §6):** 0.03 marcadores/opción en las 35 claves vs 0.05 en
  los 105 distractores; **0/35** con la clave como única sin marcador. **Opción compuesta
  como única clave (G44 §7): 0/35.**
- **Opciones textualmente duplicadas (hueco de G52 — `lot-validation.ts` verifica
  cantidad/ids/una correcta pero NO compara los textos):** auto-chequeo añadido a
  `gen.py` (`len(set(textos)) == 4`) → **0/35**.
- **Fuga entre reactivos (G38 §6 / G42 §8):** heurístico de 4-gramas sobre `(stem +
  clave)` y sobre la diagonal `(clave ↔ distractores de todo el lote)` → **0
  coincidencias no triviales**.

### 7) Inserción real — verificada en la DB

| Métrica | Antes de G53 | Después de G53 |
|---|---:|---:|
| Banco total (COUNT crudo) | 1 042 | **1 077** |
| Servibles (`isVerified=true`) | 1 038 | 1 038 |
| Retirados a propósito | 1 | 1 |
| Cola ciega (pendientes de resolución) | 0 | **35** |
| Cola canónica de discrepancias (`manualReview=null`) | 3 | 3 |
| `explanation_layers` del lote | — | **105** (3 × 35) |
| `question_source_chunks` del lote | — | **2** (Cinemática) |
| `SOURCED` en el banco | 282 | 284 (26 % — el denominador creció) |
| Física IPN FISMAT · pool | 35✓ · 0⧗ | **35✓ / 35⧗** (banco de la materia 35 → 70) |

Chequeos post-inserción (query directa): los 35 con exactamente 4 opciones y 1 correcta,
105 `explanation_layers`, clave A9/B9/C9/D8, dificultad 7/17/9/2, formato 21/12/2,
reparto por tema 2/2/2/3/3/3/3/2/3/2/2/2/3/3, `groundingStatus=SOURCED` en 2 (Cinemática)
y `TEMARIO_ONLY` en 33, `verification=null` en los 35. Cohorte con `id` prefijo
`cmtgvk…`–`cmtgvn…` del 2026-08-31 — separable por `topicId` o por timestamp para G54.

`content:coverage` en vivo: **1 038 servibles · 35 pendientes · 1 retirado · 1 074 en
banco**; meta efectiva G26 (1 222) **64 %**, brecha **438 (≈ 13 lotes)** — **no se mueve
con este lote**, pero **a diferencia de G51/G52 SÍ se moverá cuando G54 verifique**: el
pool IPN FISMAT Física (peso 20, meta efectiva 111, no compartido) pasa de 35 a ~70, los
35 caen enteros en la brecha y la descuentan 1:1 → bajará a ~403. Meta nominal de 1 500:
**69 %**.

### 8) Limpieza

El lote se compuso con un generador de Python desechable en el scratchpad
(`g53/gen.py`: los 35 reactivos con sus 3 capas, la verificación de los 21 cálculos por
dos rutas, la asignación de letra, la búsqueda del orden de inserción y los auto-chequeos
de distribución / racha cíclica / señuelo de longitud tie-aware / citas por letra /
absolutismo / opción compuesta / opciones duplicadas / fuga de 4-gramas). El generador y
los 14 archivos del lote **no se committean**; el registro permanente es
`docs/content-batches/g53-ipn-fismat-fisica.json`. `pnpm typecheck` y `pnpm lint` en
verde (cero cambios de código de producción). Cero llamadas a la API de pago.

### Siguiente (G53)

1. **Verificación ciega del lote de G53** (segunda mitad del ciclo de G2), **modelo
   Fable 5**: `pnpm content:blind-batch --topic <cada uno de los 14 topicId>` →
   `content:resolve`. **LOTE DE CÁLCULO:** `isCalcSubject("Física")` es `true`, así que
   la sesión ciega recibe `requiresCalculation:true` y **debe EJECUTAR cada cálculo con
   código real** (candado de G28), en los **21 numéricos**. **0/35 con pasaje.** Aplicar
   G36 §2 / G38 §3: recalcular el señuelo de longitud tie-aware sobre las respuestas
   ciegas y compararlo con §6. **Reactivos más apretados** (se señala cuáles, no cómo
   resolverlos, por G30 §1): los **2 `EXPERT`** (ON2 tubo abierto; FM2 fotoeléctrico al
   duplicar `f`) y **MG1** (fuerza magnética sobre una carga que viaja paralela al campo
   — la clave es 0 N, la trampa es aplicar `qvB` sin mirar el ángulo).
2. **Huecos que siguen abiertos** tras G53 (siguiente en brecha efectiva):
   - **IPN:MATEMATICAS** (pool FISMAT+MEDBIO, brecha 63) — el lote iría a FISMAT (w24,
     más contenido); recordar el follow-up de G26 §8.3 (ampliar el temario del seed de
     MEDBIO Matemáticas, hoy 6 temas sin cálculo).
   - **IPN:QUIMICA** (brecha 54, sirve a FISMAT y MEDBIO de un tiro).
   - **IPN · MEDBIO · Biología** (brecha 52).
   - **UNAM:INGLES** (brecha 33, 0 verificados, cubre A1/A2/A3).
   - **IPN SOCADM** completo (Historia de México / Universal / Geografía / Civismo).
   - **UNAM A1 Matemáticas** (114/144) y **UNAM A4 Artes** (0, tiene `SourceChunk` en 3
     temas).
3. **Auditoría 5 %: vencida, 9 ciclos.** La muestra de ids exige sesión ciega con tier
   **≠ opus-5** (el banco reciente lo aprobaron `opus-5`/`sonnet-5`).
4. **Hueco de `lot-validation.ts` de G52 sigue sin cerrar en código:** `isWellFormedOptionSet`
   no compara los textos de las opciones. G53 lo cubrió en el generador; falta llevarlo a
   `scripts/lib/lot-validation.ts` como código (`DUPLICATE_OPTIONS`), junto con el
   reactivo retenido de G52 (`cmtgtugg00001j7ewpgart1bf`).
5. **Alcance del 21-nov aún sin resolver** (G24 §7 / G26 §8.4).
6. **Heredados sin tocar:** rotación A→B→C→D de ~140 reactivos viejos
   (G3a/G3d/G13/G15), las 8 `CHART_TABLE` reclasificadas en G39, la regla de G42 §8 como
   código en `lot-validation.ts`, el rebanado de `SourceChunk` por página, y los 3
   reactivos de G37 que son paráfrasis cercanas de su guía fuente.

## G52 — Verificación ciega: Química, UNAM Área 2 (lote de G51) (2026-08-31)

**Estado: COMPLETADA.** Verificación ciega adversarial (G2) de los 35 reactivos que G51
dejó en la cola (`isVerified=false`, `verification=null`): **Química, UNAM Área 2
(Ciencias Biológicas, Químicas y de la Salud)** — materia **COMPARTIDA**
(`sharedContentKey = UNAM:QUIMICA`, A1 `questionWeight` 12 + A2 `questionWeight` 8), sus
6 temas.

**35/35 coinciden con la clave del generador (100 % de concordancia). Auto-aprobados
34/35 = 97.1 %.** El único retenido **no discrepa en la respuesta**: se retuvo porque la
lectura ciega destapó un **defecto de armado** (dos distractores idénticos) y la sesión
levantó `WEAK_DISTRACTORS` — §3. Modelo real `claude-opus-5`; G51 anunciaba Fable 5, y el
campo `model` declara el que resolvió de verdad (corrección de G17, **quinto ciclo
consecutivo** en que el modelo real y el planeado difieren, tras G44, G46, G48 y G50).

### 1. Ceguera y aislamiento

Sesión distinta de la que compuso el lote (G51). Esta sesión **nunca abrió** el commit
`3e19c7d`, el registro permanente `docs/content-batches/g51-unam-a2-quimica.json`,
`Question.options` ni la sección `## G51`; el único insumo fue el lote ciego
`scripts/content-exports/blind-batch-2026-08-31T06-09-08-664Z.json` (gitignored).
...
| Dificultad | 7 `BASIC` · 17 `INTERMEDIATE` · 9 `ADVANCED` · 2 `EXPERT` |
| `requiresCalculation` | **35/35** (`isCalcSubject("Química")` = true — el candado de G28 aplica de oficio) |
| Reactivos con pasaje | 0/35 |
| Opciones por reactivo · ids únicos | 4/4 en los 35 · 35 ids únicos |
| Institución / materia / temas | 35/35 UNAM · Química · los 6 temas de A2 |
...
### 2. Los 35 se resolvieron ejecutando el cálculo, con unidades verificadas

`isCalcSubject("Química")` es `true`, así que el candado de G28 exigía cálculo ejecutado
en **los 35**, no razonamiento en texto. El arnés (Node, en el scratchpad, no se
committea) lleva la unidad escrita en cada paso de la cadena —`u`, `mol`, `mol/L`, `mL`,
`L`, `g/mol`, `g`, `protones`— y declara explícitamente adimensionales el pH y las `Kc`
cuyos exponentes se cancelan (`[HI]²/([H₂][I₂])`). Cada resultado se contrastó contra las
cuatro opciones exigiendo **una sola coincidencia**: cero habría sido `NONE_VALID`, dos o
más `MULTIPLE_VALID`.

| Tema | Cálculo ejecutado | Resultado | Trampa descartada |
|---|---|---|---|
| Estructura atómica | promedio ponderado `62.93 u × 0.692 + 64.93 u × 0.308`; conteo p/e/n de un catión 2+ (`Z`, `A−Z`, `Z−2`); llenado de 18 e⁻ en el anión; protones `= 0.50 mol × 6.0×10²³ mol⁻¹ × 10 p⁺/molécula` | 63.55 u · 20p⁺/18e⁻/20n · `…3s²3p⁶` · 3.0×10²⁴ | **64.31 u** (promedio simple, sin ponderar) y 127.86 u (sumar las dos masas); "22 electrones" (sumar la carga en vez de restarla); `…3s²3p⁴` (el átomo neutro) |
| Tabla periódica | despeje de la abundancia `x = (70.92 − 69.72)/(70.92 − 68.93)`; escala de Pauling ordenada (Se 2.55 < S 2.58 < O 3.44 < F 3.98); tendencias de energía de ionización y de radio por carga nuclear efectiva | 60.3 % ≈ 60 % · Se<S<O<F · Ar · disminuye | **40 %** (la abundancia del *otro* isótopo); "aumenta porque también crecen masa y número atómico" |
| Ácidos y bases | `[OH⁻] = Kw/[H⁺]`; `n(HCl) = 0.025 L × 0.20 mol/L` y `V = n/0.10 mol/L`; `pH = −log₁₀[H⁺]`; `C₁V₁ = C₂V₂` con `n = 0.020 L × 6.0 mol/L` | 1.0×10⁻⁵ M · 50 mL · pH 3 · 0.24 M | **12.5 mL** (invertir la razón de concentraciones); **2.4 M** (dividir entre 50 mL) y 15 M (invertir la razón de volúmenes); pH 11 (reportar el pOH) |
| Reacciones orgánicas | relación mol-masa `1 mol C₂H₅OH : 2 mol CO₂ × 44 g/mol`; balanceo por conservación de átomos (`C: b=3`, `H: 2c=8`, `O: 2a=2b+c`) | 88 g · `C₃H₈ + 5O₂ → 3CO₂ + 4H₂O`, suma 13 | 132 g (3 mol) y 44 g (1 mol); 14 y 12 (errar el coeficiente del O₂) |
| Enlace químico | conteo de Lewis del CO₂ comprobado contra los 16 e⁻ de valencia (`4 + 2×6`); suma vectorial de dipolos en geometría lineal simétrica; `Δχ ≈ 3.2 > 1.7`; TRPEV con 2 pares libres sobre el O | 4 pares enlazantes y 4 libres · CO₂ no polar · iónico · angular ~104.5° | "2 enlazantes y 4 libres" (contar enlaces, no pares); "tetraédrica" (confundir disposición electrónica con geometría molecular) |
| Equilibrio químico | `Kc = [HI]²/([H₂][I₂]) = 2.56/0.04` en 1.0 L; `Q = [B]²/[A] = 1.0²/0.50` frente a `Kc = 4.0`; Le Chatelier por Δn(gas) (2 frente a 4 mol) y con el calor como producto | 64 · Q = 2.0 < Kc → productos · productos · Kc disminuye | **8** (no elevar `[HI]` al cuadrado); "hacia reactivos porque Q < Kc" (la dirección invertida); "Kc no cambia con la temperatura" |

Confianza declarada: **0.94 mínima** (el conteo de pares de Lewis del CO₂), 0.96–0.98 en
el resto. Los reactivos conceptuales no se resolvieron "a ojo": detrás de cada uno hay un
cálculo o un conteo ejecutado (llenado electrónico, escala de Pauling, balance de e⁻ de
valencia, Δn de gases, `Q` frente a `Kc`) más un predicado sobre el **contenido** de la
opción, con la misma exigencia de unicidad que la vía numérica. Los distractores se
descartaron siempre por lo que dicen, nunca por su letra ni su posición.

### 3. Defecto real hallado: un reactivo con dos opciones idénticas

Es el **primer defecto de armado en cinco ciclos de verificación** (G44, G46, G48, G50
cerraron con cero `problems`), y lo destapó justamente la lectura ciega, que obliga a
leer las cuatro opciones como las leería un sustentante.

- **Reactivo:** `cmtgtugg00001j7ewpgart1bf` — Ácidos y bases, `PROBLEM_SOLVING`,
  "disolución con pH 9 a 25 °C, ¿cuál es su `[OH⁻]`?".
- **Defecto:** **dos distractores son textualmente idénticos** (`1×10⁻⁹ M` aparece en dos
  incisos), así que el reactivo ofrece **3 alternativas distintas, no 4**. El sustentante
  puede descartar ambos de golpe: el azar sube de 25 % a 33 %.
- **La clave no está afectada:** `[OH⁻] = Kw/[H⁺] = 1.0×10⁻¹⁴ / 1.0×10⁻⁹ = 1.0×10⁻⁵ M`
  es única en el conjunto y la sesión la eligió; por eso el reactivo **coincide** con el
  generador y aun así **no se publica**. La resolución de G2 se comporta como debe:
  `problems` no vacío ⇒ `UNPUBLISHED`, con el veredicto completo adjunto en
  `Question.verification` para el panel de discrepancias (F3).
- **Arreglo pendiente:** reemplazar uno de los duplicados por un distractor real (p. ej.
  `1×10⁻¹⁴ M`, de confundir `Kw` con la concentración) y volver a pasarlo por el canal
  ciego. **No borrar el reactivo.**

**Hueco de pipeline que esto expone.** `lot-validation.ts` **no atrapa el caso**:
`isWellFormedOptionSet` (líneas 113-119) verifica cantidad (4), ids (`A/B/C/D`) y que
haya exactamente una `isCorrect`, pero **nunca compara los TEXTOS de las opciones**. Un
lote con opciones duplicadas pasa `content:validate-batch` y el paso obligatorio de
`content:insert` sin una sola advertencia. Es corregible con tres líneas
(`new Set(options.map(o => o.text.trim())).size === 4`, código nuevo `DUPLICATE_OPTIONS`).
**No se implementó aquí** porque G52 es una fase de verificación y el encargo pedía cero
cambios de código de producción; queda como la recomendación de mayor palanca para G53.

### 4. Resolución

| Resultado | n |
|---|---|
| ✔ Auto-aprobados (coincide + `confidence ≥ 0.85` + `problems` vacío) | **34** |
| ✋ Sin publicar con veredicto | **1** (`WEAK_DISTRACTORS`, §3) |
| ⚠️ Omitidos | **0** |
| Letra elegida (traducida) == letra del generador | **35/35** |

La distinción importa para leer la tasa: **la concordancia de respuesta fue 100 %**; el
97.1 % de auto-aprobación mide la calidad del **armado**, no la del criterio de respuesta.
...
Distribución sana, y **los 6 temas usan las 4 letras** (`ABDBDC`, `DCBAA`, `CCBDBA`,
`BADCBC`, `BCADAD`, `ACADBC`) — cero corridas cíclicas A→B→C→D y ningún tema con la clave
concentrada. Las letras que **esta** sesión eligió sobre el lote barajado (A9/B10/C8/D8)
no tienen por qué parecerse: el barajado es por reactivo y sembrado con su `id`.

### 6. Higiene del canal ciego: la línea 3 sigue filtrando

Igual que en G48 §6 y G50 §6, la lectura de estado inicial se cortó **antes** de que la
línea 3 expusiera los agregados de G51. Esta sesión no vio la clave A/B/C/D agregada ni
ningún valor numérico del lote; los 35 se calcularon desde cero y el requisito de
coincidencia única habría fallado si la composición hubiera errado alguno.

**La deuda persiste y esta vez tiene nombre y apellido:** la línea 3 de G51 **sí
inventaría los reactivos más apretados y sus ids internos** ("los 2 EXPERT: EA5 protones
en 0.50 mol; EQ5 Q vs Kc; EA2 masa atómica ponderada; AB2 pH→pOH"). Aunque no da las
respuestas, **le entrega a la sesión ciega el mapa de dónde están las trampas**, que es
exactamente lo que G48 §6 pidió no hacer. El arreglo sigue siendo de la fase que compone:
ni la sección `## GNN` ni la línea 3 deben inventariar los ítems del lote.

### 7. Acumulado real (DB en vivo, antes → después)

| Métrica | Antes de G52 | Después de G52 |
|---|---|---|
| Banco total (COUNT crudo) | 1 042 | **1 042** |
| Servibles (`isVerified=true`) | 1 004 | **1 038** |
| Cola ciega (`verification=null`) | 35 | **0** |
| Cola canónica de discrepancias | 2 | **3** (los 2 de G46 + el de §3) |
| Retirados a propósito | 1 | 1 |
| Auto-aprobación global | 99.8 % (1 005/1 007) | **99.7 % (1 039/1 042)** |
| UNAM A2 Química | 72✓ / 35⧗ | **106✓ / 0⧗ / 1✋** |
| `SOURCED` en banco | 282 (27 %) | 282 (27 %) — los 35 son `TEMARIO_ONLY` |

Cobertura por tema de Química UNAM A2 (consulta en vivo, después): Estructura atómica
**18** · Enlace químico **18** · Equilibrio químico **18** · Reacciones orgánicas **18** ·
Ácidos y bases **17** (de 18 insertados; el 18.º es el retenido) · Tabla periódica **17**.
El reparto de G51 cumplió su objetivo: los 6 temas salieron de 12 parejos y quedaron en
17-18 parejos.

`content:coverage`: **1 038 servibles · 0 pendientes de resolución · 1 retirado · 1 039 en
banco** (no cuenta los 3 de discrepancias). Meta efectiva G26 (1 222): **64 % → 64 %**,
brecha **438 → 438 (≈ 13 lotes)** — **no se movió ni un reactivo**, exactamente como G51
predijo: el pool compartido `UNAM:QUIMICA` pasa de 123 a **157 verificados** (A1 51 + A2
106) frente a una meta de grupo de **67**, así que los 34 aprobados cayeron enteros en un
pool ya rebasado y dan **profundidad, no descuento de brecha**. Es el reverso exacto de
G45 (donde los 35 descontaban 1:1). Meta nominal 1 500: **67 % → 69 %**.

### 8. Criterios de aceptación

- [x] Nunca se vio la respuesta correcta antes de responder — solo el lote ciego, 0
      fugas de `isCorrect`/`explanation`, sin abrir commit, JSON del lote ni `## G51`.
- [x] Cálculos **ejecutados con unidades verificadas** en los 35 (arnés en Node con la
      unidad escrita en cada paso, adimensionales declarados, y coincidencia única
      obligatoria contra las cuatro opciones).
- [x] `pnpm typecheck` y `pnpm lint` en verde.
- [x] Cero cambios de código de producción; cero llamadas a la API de pago (el arnés vive
      en el scratchpad; el lote ciego y el archivo de respuestas están bajo
      `scripts/content-exports/`, gitignored).

### 9. Deuda y recomendación para G53

- **Lo más barato y de mayor palanca: cerrar el hueco de §3 en `lot-validation.ts`**
  (textos de opción distintos, código `DUPLICATE_OPTIONS`) y, de paso, reparar el
  reactivo retenido y devolverlo al canal ciego. Es media hora y evita que el defecto se
  repita en los ~13 lotes que faltan.
- **`UNAM:QUIMICA` queda cerrado** (157 verificados vs meta de grupo 67): no volver a
  componer química de la UNAM hasta que el resto del banco alcance. Lo mismo aplica a
  UNAM A1 Física (107 vs 89) y A2 Biología tras G48.
- **UNAM A1 Matemáticas sigue siendo el hueco STEM más caro** (114 vs meta ~144): un lote
  entero cae dentro de la brecha efectiva, sin desperdicio. Es la recomendación para el
  próximo lote.
- **Auditoría 5 % vencida, ya 9 ciclos**: exige un tier ≠ el que aprobó. El banco reciente
  lo aprobó `opus-5`/`sonnet-5`, así que un `fable-5` o `opus-4-8` la habilitaría.
- Siguen abiertos: UNAM A4 Artes, IPN SOCADM completo, los otros pools STEM de alto peso
  de G39 §7, la rotación A→B→C→D de ~140 reactivos viejos, la regla de G42 §8 como código
  en `lot-validation.ts`, los 3 reactivos de G37 que parafrasean su guía, y la regla de
  G48 §6 (que G51 volvió a incumplir — ver §6).
- El alcance del 21-nov sigue **sin respuesta del dueño, veinte fases después**.
...
**Estado: COMPLETADA.** Verificación ciega adversarial (G2) de los 35 reactivos que G49
dejó en la cola (`isVerified=false`, `verification=null`): **Física, UNAM Área 1
(Ciencias Físico-Matemáticas y las Ingenierías)** — `questionWeight` 16, materia NO
compartida (`sharedContentKey` NULL), 10 de sus 12 temas.

**35/35 coinciden con la clave del generador. Auto-aprobados 35/35 = 100 %.** Cero
`problems`, cero discrepancias, cero retenidos. Modelo real `claude-opus-5` — el plan de
G49 anunciaba Fable 5; el campo `model` declara el que resolvió de verdad (corrección de
G17, **cuarto ciclo consecutivo** en que el modelo real y el planeado difieren, tras G44,
G46 y G48).

### 1. Ceguera y aislamiento

Sesión distinta de la que compuso el lote (G49). Esta sesión **nunca abrió** el commit
`394d723`, el registro permanente `docs/content-batches/g49-unam-a1-fisica.json`,
`Question.options` ni la sección `## G49`; el único insumo fue
`scripts/content-exports/g50-blind.json` (gitignored).
...
| Dificultad | 7 `BASIC` · 17 `INTERMEDIATE` · 9 `ADVANCED` · 2 `EXPERT` |
| `requiresCalculation` | **35/35** (`isCalcSubject("Física")` = true — el candado de G28 aplica de oficio, a diferencia de G48) |
| Reactivos con pasaje | 0/35 |
| Opciones por reactivo · ids únicos | 4/4 en los 35 · 35 ids únicos |
| Institución / materia / temas | 35/35 UNAM · Física · los 10 temas reforzados |
| Clave en la MISMA posición tras barajar | **6/35** (azar ≈ 8.75) — el barajado determinista de `blind-verification.ts` sí esconde la letra original |

### 2. Los 35 se resolvieron ejecutando el cálculo, con unidades verificadas

`isCalcSubject("Física")` es `true`, así que el candado de G28 exigía cálculo ejecutado en
**los 35**, no razonamiento en texto. El arnés (Python + `sympy.physics.units`, en el
scratchpad, no se committea) hace tres cosas por reactivo:

1. **Construye la magnitud con unidades SI** y la convierte con `convert_to` a la unidad
   que pide el enunciado. `as_number_in()` **falla ruidosamente si la dimensión no
   cuadra**, así que un error de unidades no puede pasar como número correcto.
2. **Parsea las cuatro opciones desde LaTeX** y descarta por dimensión las
   incompatibles antes de comparar.
3. **Exige coincidencia única**: `assert len(hits) == 1`. Cero coincidencias sería
   `NONE_VALID`; dos o más, `MULTIPLE_VALID`. Los 35 dieron exactamente una.

| Tema | Cálculo ejecutado | Resultado | Trampa descartada |
|---|---|---|---|
| Conservación de momento | choque inelástico `v=m₁v₁/(m₁+m₂)`; retroceso con `p_total=0`; impulso `Δp=m|v_f−v_i|` | 2.4 m/s · 0.8 m/s · 3 N·s | 12 m/s (no dividir entre la masa total); **1.5 N·s** (olvidar que el rebote invierte el signo) |
| Conservación de momento (conceptual) | `K=p²/(2m)` con `p` fija: 240 kJ (1 200 kg) vs 36 kJ (8 000 kg) | el de menor masa | "el camión, porque pesa más" |
| Fluidos | `p=ρgh`; Pascal `F₁/A₁=F₂/A₂`; continuidad `A₁v₁=A₂v₂` (gasto verificado igual en ambas secciones) | 40 000 Pa · 400 N · 15 m/s | 400 000 Pa (una potencia de 10); 2.4 m/s (invertir la razón de áreas) |
| Fluidos (flotación) | fracción sumergida `ρ_cuerpo/ρ_fluido` = 0.60 y `E = W` = 60 N en equilibrio | densidad menor que la del agua | "el empuje **supera** al peso" — falso en equilibrio |
| Óptica | ángulo dado respecto a la **superficie**: normal = 90−20; trazado de rayos sobre `y=x²/(4f)` con la fuente en el foco (dirección reflejada (0,1) en los tres `x₀` probados); lente `1/f=1/dₒ+1/dᵢ` con `dₒ<f` → `dᵢ=−0.10 m`, `M=+2` | 70° · haz paralelo · virtual, derecha, mayor | **20°** (leer el dato como si ya fuera respecto a la normal) |
| Óptica (espejo plano) | imagen virtual simétrica: separación persona-imagen `= 1.5 + 1.5` | 3 m y virtual | 1.5 m (distancia al **espejo**, no a la imagen); "real" |
| Termodinámica | `Q=mcΔT` (ΔT de 30 °C = 30 K); `e=1−Q_c/Q_h`; adiabática `TV^(γ−1)=cte` (γ=5/3, V a la mitad): 300 K → 476.2 K | 252 000 J · 25 % · la temperatura sube | **75 %** (reportar `Q_c/Q_h`); "permanece constante porque Q=0" |
| Ondas y sonido | `λ=c/f` con 60 MHz → 6×10⁷ s⁻¹; energía ∝ A²: `(2A)²/A² = 4` | 5 m · se cuadruplica | 0.2 m (invertir la razón); "se duplica, igual que la amplitud" |
| Gravitación universal | `g=GM/R²` evaluada simbólicamente con M→2M, R→2R: `g'/g = 2/4`; Kepler `T∝r^{3/2}`: `4^{3/2}`; `g` a 400 km = 8.68 m/s² (89 % de la superficial) | la mitad · 8 veces · caída libre continua | "el doble" (olvidar el cuadrado del radio); **16** = 4² y **64** = 4³; "ya casi no hay gravedad allá arriba" |
| Electrostática | `E=F/q`; `E_r=kq/r²` con q<0 → componente radial **negativa**; contacto entre conductores idénticos resuelto simbólicamente exigiendo que la opción **conserve la carga total Y sea simétrica** | 5 N/C · hacia la carga · Q/2 en cada una | 20 N/C (multiplicar en vez de dividir); "Q en cada una" (duplicaría la carga) |
| Circuitos eléctricos | `P=I²R` (comprobado por la vía `V=IR=20 V → P=VI`); `E=Pt` en watt-hora; paralelo `1/R_eq=1/R₁+1/R₂` = 5 Ω < 10 Ω | 80 W · 300 Wh · disminuye | **20 W** (quedarse en `V=IR`); 18 000 Wh (mezclar horas y segundos); "aumenta porque hay más resistores" |
| Magnetismo | `F=qvB sen 90°` (dimensión C·(m/s)·T reducida a N); Faraday `fem=−dΦ/dt` evaluada en Φ constante (0) y Φ variable (≠0); balance `τω = fem²/R` = 20 W | 6 N · flujo variable · mecánica→eléctrica | 1.5 N; "espira inmóvil en campo constante"; el sentido inverso (motor) |
| Física moderna | Bohr `E_n=−13.6/n²`: n=3→2 libera 1.89 eV = 656 nm (H-α); De Broglie con 100 V: `λ=h/p` = 123 pm | emisión al bajar de nivel · comportamiento ondulatorio | "absorbe un fotón" (eso **sube** de nivel); "la luz es corpuscular" (cierto, pero no es lo que demuestra difractar **electrones**) |

`usedCalculation:true` se declaró con verdad en **los 35**. Los ítems conceptuales no se
resolvieron "a ojo": cada uno lleva detrás un cálculo ejecutado (trazado de rayos,
ecuación de la lente, `TV^(γ−1)`, `g` a 400 km, De Broglie, balance de potencia del
generador, `K=p²/2m`, `R_eq` en paralelo) más un predicado sobre el texto de la opción
con la **misma exigencia de unicidad** que la vía numérica. Los distractores se
descartaron siempre **por su contenido**, nunca por su letra ni su posición.

### 3. Defecto de herramienta hallado y corregido dentro de la sesión

El primer arnés se escribió con un heredoc de Bash que **colapsó los backslashes**: el
patrón `\text{...}` llegó al `re.sub` como `\t` + `ext`, es decir **un TABULADOR**, y la
sustitución nunca disparaba. El síntoma fue limpio —la unidad quedaba como
`\text{m/s}`, no existía en la tabla de unidades y el ítem 1 reportaba **cero**
coincidencias— justo el `assert` de unicidad haciendo su trabajo. Correcciones:

- Los archivos con regex se escriben con la herramienta `Write`, no por heredoc.
- El backslash del patrón va con `re.escape()`, nunca crudo.
- **`\,` de LaTeX es a la vez separador de miles (`400\,000`) y espacio número-unidad
  (`4\,\text{m}`)**: se elimina **DESPUÉS** de resolver `\text{}` y `\times10^{n}`, o
  `1.8\times10^{16}` se parsea como `1.8`. (Riesgo ya anotado en G45/G49; aquí quedó
  cubierto por código y por prueba de humo sobre 5 reactivos antes de la corrida.)
...
Confianza declarada: **0.97 en los 18** resueltos por comparación numérica directa,
**0.96 en los 17** resueltos por cálculo de respaldo + predicado. Ninguno por debajo de
0.96. Cero `problems` de cualquier tipo.
...
Distribución sana. Por tema, los 9 temas con ≥3 reactivos usan ≥3 letras distintas;
el único con 2 reactivos (Física moderna) usa 2 letras distintas — **cero corridas
cíclicas A→B→C→D** y ningún tema con la clave concentrada. Las letras que **esta**
sesión eligió sobre el lote barajado (A12/B7/C8/D8) no tienen por qué parecerse: el
barajado es por reactivo y sembrado con su `id`.

### 6. Higiene del canal ciego: la línea 3 (G48 §6)

La lectura de estado inicial se cortó **antes** de que la línea 3 expusiera los agregados
de G49; esta sesión no vio la clave A/B/C/D agregada ni ningún valor numérico del lote.
La memoria de sesiones previas sí conservaba el agregado `A9/B9/C9/D8` de G49, y se
declara: **no se usó**, y de todos modos es inservible para las etiquetas ciegas porque
el rebarajado va sembrado por `questionId` (mismo argumento que G44/G46/G48). Los 35
valores se **calcularon desde cero**; el `assert` de unicidad habría fallado si la
composición hubiera errado alguno.

**La regla sigue sin implementarse:** la sección `## GNN` de composición no debe
inventariar los valores de los ítems, y la línea 3 tampoco. El arreglo es de la fase que
compone.

### 7. Acumulado real (DB en vivo, antes → después)

| Métrica | Antes de G50 | Después de G50 |
|---|---|---|
| Banco total (COUNT crudo) | 1 007 | **1 007** |
| Servibles (`isVerified=true`) | 969 | **1 004** |
| Cola ciega (`verification=null`) | 35 | **0** |
| Cola canónica de discrepancias | 2 | **2** (los 2 retenidos de G46) |
| Retirados a propósito | 1 | 1 |
| Auto-aprobación global | 99.8 % (970/972) | **99.8 % (1 005/1 007)** |
| UNAM A1 Física | 72✓ / 35⧗ | **107✓ / 0⧗ / 0✋** (⚓46/61) |
| `SOURCED` en banco | 282 (28 %) | 282 (28 %) — los 16 del lote entraron con G49 |

Cobertura por tema de Física UNAM A1 (consulta en vivo, después): Cinemática y dinámica
11 · Trabajo y energía 7 · **Conservación de momento 7** · Gravitación universal 9 ·
Fluidos 9 · Termodinámica 10 · Ondas y sonido 10 · Óptica 9 · Electrostática 9 ·
Circuitos eléctricos 9 · Magnetismo 9 · Física moderna 8. **Los 10 temas que G49
priorizó quedaron reforzados exactamente como se planeó** y el peor de todos
(Conservación de momento, 2) sale del sótano.

`content:coverage`: **1 004 servibles · 0 pendientes de resolución · 1 retirado · 1 005 en
banco** (no cuenta los 2 de discrepancias). Meta efectiva G26 (1 222): **63 % → 64 %**,
brecha **455 → 438 (≈ 13 lotes)**. Meta nominal 1 500: **65 % → 67 %**. La celda de
Física UNAM A1 tenía meta 89 (`questionWeight` 16 × densidad 1 500/270): estaba en 72
(−17) y **queda en 107 (+18)**, así que 17 de los 35 descontaron brecha efectiva y 18
cayeron en un pool que ya rebasó su meta.

### 8. Criterios de aceptación

- [x] Nunca se vio la respuesta correcta antes de responder — solo el lote ciego, 0
      fugas de `isCorrect`/`explanation`, sin abrir commit, JSON del lote ni `## G49`.
- [x] Cálculos **ejecutados con unidades verificadas** en los 35 (`sympy.physics.units`,
      conversión con `convert_to` que falla si la dimensión no cuadra, descarte de
      opciones por dimensión y coincidencia única obligatoria).
- [x] `pnpm typecheck` y `pnpm lint` en verde.
...
### 9. Deuda y recomendación para G51

- **UNAM A1 Física queda cubierto** (107 vs meta 89) — el pool sale de la lista de
  huecos, igual que A2 Biología tras G48.
- **UNAM A1 Matemáticas sigue siendo el hueco STEM más caro** (114 vs meta ~144): un
  lote entero cae dentro de la brecha efectiva, sin desperdicio.
- **Auditoría 5 % vencida** (contada en 8 ciclos ya): exige un tier ≠ el que aprobó. La
  mayoría del banco reciente la aprobó `opus-5`/`sonnet-5`, así que un `opus-4-8` o
  `fable-5` la habilitaría.
- Siguen abiertos: UNAM A4 Artes, IPN SOCADM completo, los otros pools STEM de alto peso
  de G39 §7, la rotación A→B→C→D de ~140 reactivos viejos, la regla de G42 §8 como
  código en `lot-validation.ts`, los 3 reactivos de G37 que parafrasean su guía, y la
  regla de G48 §6 (no inventariar valores en la sección de composición ni en la línea 3).
- El alcance del 21-nov sigue **sin respuesta del dueño, dieciocho fases después**.
