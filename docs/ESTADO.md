...
Última actualización: 2026-08-21 · Última fase ejecutada: R7 (COMPLETADA — **dominio definitivo confirmado: `yaentre.com`** comprado 21-ago-2026 en Akky; barrido exhaustivo confirma 0 residuos de `yaentre.mx`/`acierta.mx` fuera de contenido histórico ya congelado, ver fila R7)

## URL de producción actual

**`https://acierta.vercel.app`** (asignada automáticamente por Vercel — proyecto `acierta`, org `angel011298s-projects`). El nombre del proyecto en Vercel sigue siendo `acierta` y **no se renombró en el rebrand** a propósito: `.vercel/project.json` es autogenerado y renombrar el proyecto cambia esta URL, lo que obliga a rehacer `NEXT_PUBLIC_SITE_URL` y el webhook de Stripe. **El dominio propio ya está comprado — `yaentre.com`, adquirido el 21 de agosto de 2026 en Akky** (orden `20260821697888`, 1 año); falta apuntarlo por DNS a Vercel (fase R6). Ese cambio es de un solo campo (`NEXT_PUBLIC_SITE_URL` + webhook de Stripe, ver `docs/STRIPE_LIVE_CHECKLIST.md` §3) sobre este mismo despliegue, sin repetir nada de G7.
