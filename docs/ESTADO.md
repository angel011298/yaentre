...
Última actualización: 2026-08-22 · Última fase ejecutada: R6 (PARCIAL — `yaentre.com`/`www.yaentre.com` ya agregados al proyecto de Vercel vía CLI; **falta que alguien con acceso a Akky pegue 2 registros A** para que el DNS resuelva — receta exacta en la fila R6)

## URL de producción actual

**`https://acierta.vercel.app`** (asignada automáticamente por Vercel — proyecto `acierta`, org `angel011298s-projects`). El nombre del proyecto en Vercel sigue siendo `acierta` y **no se renombró en el rebrand** a propósito: `.vercel/project.json` es autogenerado y renombrar el proyecto cambia esta URL, lo que obliga a rehacer `NEXT_PUBLIC_SITE_URL` y el webhook de Stripe. **El dominio propio ya está comprado y agregado al proyecto de Vercel — `yaentre.com` y `www.yaentre.com`** (adquirido el 21 de agosto de 2026 en Akky, orden `20260821697888`; agregado a Vercel el 22 de agosto de 2026, fase R6). **Único paso que falta para que resuelva:** entrar al panel DNS de Akky (requiere la cuenta/correo de la compra, `angelortizsanchez0112@gmail.com` — esta sesión no tiene esa credencial) y agregar:

```
A   yaentre.com       76.76.21.21
A   www.yaentre.com   76.76.21.21
```

Vercel emite el certificado SSL solo y avisa por correo cuando termina — puede tardar minutos u horas según la propagación DNS. Una vez que resuelva, el cambio de `NEXT_PUBLIC_SITE_URL` + webhook de Stripe sigue siendo de un solo campo (ver `docs/STRIPE_LIVE_CHECKLIST.md` §3) sobre este mismo despliegue, sin repetir nada de G7 — deliberadamente no se tocó todavía (ver fila R6, evita romper los correos de verificación del sitio que hoy sí funciona).
