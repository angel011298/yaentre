// Construye los 6 archivos JSON del lote G89 (uno por tema priorizado de
// UNAM Área 1) a partir de build-data.mjs. La posición de la respuesta
...
const byTopic = new Map(); // topicIndex(1-6) -> array of drafts
...
for (const item of ITEMS) {
  ...
  const draft = {
    ...
    explanations: [
      ...
      { layer: 2, title: 'Resolución paso a paso', content: item.layer2, ...(item.latexContent2 ? { latexContent: item.latexContent2 } : {}) },
...
const TOPIC_FILE_NAMES = [
  '1-progresiones-combinatoria.json',
  '2-estadistica-descriptiva.json',
  '3-series-sucesiones.json',
  '4-matrices-sistemas-ecuaciones.json',
  '5-conservacion-momento.json',
  '6-trabajo-energia.json',
];
