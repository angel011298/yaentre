...
4. **Dominio de envío — bloqueo real sin dominio propio:** el código usa
   `Acierta <notificaciones@acierta.mx>` como remitente
   ...
   verificar `acierta.mx` porque **el dominio aún no se compra** (mismo
   ...
   - **Recomendado, temporal:** cambia `FROM_ADDRESS` en
     ...
     propio — deshace este cambio cuando conectes `acierta.mx`.
   - Sin ese cambio, cualquier envío con `@acierta.mx` fallará con "domain
...
5. **Configura Supabase Auth para usar este SMTP** (esto es lo que de
   ...
   Sender email: onboarding@resend.dev (o notificaciones@acierta.mx una vez verificado el dominio)
   ```
